...
function GenerateFromTemplateDialog({ lease }: { lease: LeasePreview }) {
    ...
    useEffect(() => {
        if (open) {
            api.get<{ data: Template[] }>('/api/leases/templates')
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2 flex-wrap">
                            ...
                            {pendingPayments.length > 0 && (
                                ...
                                >
                                    <AlertTriangle size={12} /> {pendingPayments.length}{' '}
                                    {pendingPayments.length === 1 ? 'pago pendiente' : 'pagos pendientes'}
                                </Badge>
                            )}
                        </h1>
                    ...
                    {lease.status === 'ACTIVE' && (
                        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                            <LeaseRenewalSection
                                ...
                                noticeRequiredDays={
                                    (lease as unknown as { noticeRequiredDays: number }).noticeRequiredDays
                                }
                                earlyTerminationPenalty={
                                    (lease as unknown as { earlyTerminationPenalty: number }).earlyTerminationPenalty
                                }
                            />
                            <AlertDialog>
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            Esta acción terminará el contrato y marcará la unidad como vacante. Esta
            ...
            {lease.status === 'ACTIVE' && daysUntilExpiry <= 60 && (
                <div
                    className={`p-4 rounded-lg border flex items-start gap-3 ${daysUntilExpiry <= 30 ? 'bg-red-50 border-red-100 text-red-800' : 'bg-amber-50 border-amber-100 text-amber-800'}`}
                >
                    <AlertTriangle className="mt-0.5 flex-shrink-0" size={20} />
                    <div>
                        <h3 className="font-bold">Este contrato vence pronto</h3>
                        <p className="text-sm opacity-90">
                            Quedan <strong>{daysUntilExpiry} días</strong> para finalizar (Fecha fin:{' '}
                            {formatDateUTC(lease.endDate)}). Considera contactar al inquilino para renovar o coordinar
                            la entrega.
                        </p>
                    </div>
                </div>
                ...
                <div className="p-4 rounded-lg border bg-red-50 border-red-100 text-red-800 flex items-start gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm opacity-90 mb-2">
                            ...
                            {pendingPayments.length === 1 ? 'cobro pendiente' : 'cobros pendientes'} por gestionar.
                        </p>
                        <ul className="text-sm space-y-2">
                            {pendingPayments.map((pending: PendingPayment) => (
                                <li
                                    key={pending.id}
                                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2 bg-white rounded border border-red-100"
                                >
                                    <div className="flex flex-col">
                                        <span className="font-semibold text-gray-900">
                                            {pending.description ||
                                                `Arriendo ${new Date(pending.periodStart).toLocaleDateString('es-CO', { month: 'long' })}`}
                                        </span>
                                        <span className="text-xs text-gray-500">
                                            Vence:{' '}
                                            {new Date(pending.paymentDate).toLocaleDateString('es-CO', {
                                                dateStyle: 'medium',
                                            })}
                                        </span>
                                    </div>
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-1">
                                ...
                                <p className="font-medium flex items-center gap-2">
                                    <Calendar size={16} className="text-gray-400" /> {formatDateUTC(lease.startDate)}
                                </p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-gray-500 uppercase">Fin</label>
                                <p className="font-medium flex items-center gap-2">
                                    <Clock size={16} className="text-gray-400" /> {formatDateUTC(lease.endDate)}
                                </p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-medium text-gray-500 uppercase">Canon Mensual</label>
                                <p className="font-medium text-lg text-indigo-700">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="font-medium">
                                    {lease.securityDeposit ? formatCurrency(lease.securityDeposit) : 'No aplica'}
                                </p>
                            </div>
                            {lease.notes && (
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                                        ...
                                        {lease.contractPdfPath ? (
                                            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto sm:items-center">
                                                <Badge
                                                    variant="outline"
                                                    className="bg-green-50 text-green-700 border-green-200 justify-center sm:justify-start w-full sm:w-auto"
                                                >
                                                    Subido
                                                </Badge>
                                                <div className="flex gap-2 w-full sm:w-auto">
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={async () => {
                                                            if (
                                                                confirm(
                                                                    '¿Está seguro de eliminar el documento del contrato? Esta acción eliminará el archivo permanentemente.',
                                                                )
                                                            ) {
                                                                try {
                        ...
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-sm text-gray-500 mt-1">
                                    {lease.unit?.property?.name || 'Propiedad Principal'}
                                </p>
                                <Button
                                    variant="link"
                                    ...
                                    <span className="font-bold text-indigo-900">
                                        {
                                            ...
                                            differenceInMonths(
                                                addDays(new Date(lease.endDate), 1),
                                ...
                                {payments?.map((payment) => (
                                    <PaymentCard
                                        ...
                                        onDelete={(id) => {
                                            if (confirm('¿Está seguro de eliminar este cobro pendiente?')) {
                                                paymentTrackingApi
                                                    .deletePayment(Number(id) ? String(id) : id) // Handle potentially numeric id? No, usually string. Safest to just pass id.
                                                    .then(() => {
                                                        toast.success('Cobro eliminado exitosamente');
                                                        window.location.reload();
                                                    })
