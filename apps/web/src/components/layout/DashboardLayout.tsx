...
interface SidebarContentProps {
    onClose?: () => void;
    navItems: { label: string; href: string; icon: React.ComponentType<{ className?: string }>; hidden?: boolean }[];
    currentPath: string;
    onLogout: () => void;
}

const SidebarContent = ({ onClose, navItems, currentPath, onLogout }: SidebarContentProps) => (
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-gray-50">
            ...
            <aside className="hidden md:flex md:flex-col md:w-64 bg-white border-r border-gray-200">
                <SidebarContent navItems={navItems} currentPath={location.pathname} onLogout={logout} />
            </aside>
